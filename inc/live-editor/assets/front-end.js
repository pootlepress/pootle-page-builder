...
jQuery( function ( $ ) {
	var $addRowDialog, $body, $contentPanel, $deleteDialog, $deletingWhat, $designTemplateDialog, $designTemplatePreview,
		$iconPicker, $loader, $mods, $panels, $postSettingsDialog, $ppb, $ppbIpadColorDialog, $rowPanel, $setTitleDialog,
		$tooltip, applyDesignTemplate, dialogAttr, pickFaIcon, prevu;
	$.each( ppbData.grids, function ( i, v ) {
	...
	prevu = {
		...
		contentDraggable: {
			...
			drag: function ( e, ui ) {
				...
				hiMrgn = (
									 parseInt( $ro.css( 'min-height' ) ) - $t.outerHeight()
								 ) / 2;
				wiMrgn = (
									 $p.width() - parseInt( $t.outerWidth() )
								 ) / 2;
				if ( top < - 25 || left < - 25 ) {
		...
		insertImage: function () {
			...
			prevu.insertImageFrame.on( 'select', function () {
				...
				$img = '<figure id="attachment_' + img.id + '" class="' + (
	...
	dialogAttr.open = function ( event, ui ) {
		return $( this ).find( '.templates-wrap' ).masonry();
	};
...
//	prevu.tmce.theme = 'ppbprevu';
	...
	applyDesignTemplate = function ( e ) {
		...
		if ( $target.hasClass( 'fa-search' ) ) {
			...
			return $designTemplateDialog.ppbDialog( 'close' );
		}
	};
...
ppbTemplateFromRow = function ( rowI, thumb ) {
	...
	for ( j = 0, len = ref.length; j < len; j ++ ) {
		cb = ref[j];
		if ( cb && cb.info && parseInt( cb.info.grid ) === rowI ) {
			parseContent( cb );
		}
	}
	ref1 = ppbData.grid_cells;
	for ( k = 0, len1 = ref1.length; k < len1; k ++ ) {
		cell = ref1[k];
		if ( cell && parseInt( cell.grid ) === rowI ) {
			parseCell( cell );
		}
	}
